...
// per session with five kept.
//
// Safe to ship where Log/LogChannel are not: redscript compiles every installed mod into ONE
// unit, so the `native func` declaration a Logs.reds carries is global, and two mods each
...
// download it by hand when they decline RedHttpClient - so it also appears in the README, the
// Nexus description and the curl command.
//
// Renamed from `locations_full.json` before 1.0.0 shipped, while there was no installed base to
// break; the `_full` recorded the API's old slim/full split, which no longer exists. AFTER
// 1.0.0 a change here needs a migration read of the old name, not a rename.
...
public class NCZoningService extends ScriptableService {
  ...
  public func LoadCache() -> Bool {
    // Each early return records a reason, so the failure the player sees separates "never
  ...
  // Registry locations whose server-computed district matches (e.g. "Watson"). The consumer
  // supplies the district in API vocabulary; joining that to the player's live district is the
  // consumer's job (game DistrictManager + a normalization map).
